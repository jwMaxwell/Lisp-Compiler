...
node_ptr build_AST(std::vector<token_t> &tokens, node_ptr ast) {
  ...
  if (token.value == "(" && token.type == "symbol") {
    ...
    std::dynamic_pointer_cast<Expression>(ast)->children.push_back(
        build_AST(tokens, newExpression));
    ...
    for (size_t i = 0; i < parent->children.size(); ++i) {
      ...
      if (child->get_class_name() == "expression") {
        ...
        if (literalChild->get_value() == "'" &&
            literalChild->get_type() == "identifier") {
          literalChild->set_value("quote");
          if (i + 1 < parent->children.size()) {
            std::vector<node_ptr> newQuoteChildren = {child,
                                                      parent->children[i + 1]};
            node_ptr newExpression = std::make_shared<Expression>();
            std::dynamic_pointer_cast<Expression>(newExpression)->children =
                newQuoteChildren;
...
node_ptr parse(std::vector<token_t> &tokens) {
  ...
  container->children = std::dynamic_pointer_cast<Expression>(ast)->children;
